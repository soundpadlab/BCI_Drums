function dataToSend = trainingModule_with_tempo(handles)
persistent smileData clenchData furrowData browData blinkData relaxData;

timeSlice = 1; %record window
tempo = 40;
rest = 60/tempo - timeSlice;
[y, Fs] = audioread('audio\snare.wav');
...
while exist('_trigger','file') == 2
    ...
    while (toc<=timeSlice) && (exist('_trigger','file') == 2)
        raw = char(step(hudpr)');
        data = strsplit(raw, ' ');
        set(handles.status, 'String', strcat(userData,' on beat!'));

        if (toc>=timeSlice/2 && playedOnce==0)
%                 soundsc(y,Fs);
%                 soundsc(4400,4400);
                play(player);
                playedOnce = 1;
    ...
    if (exist('_trigger','file') == 2)
        switch userData
            ...
            %capture data doing nothing / ground truth
            case 'Relax'
                x = size(dataToSave);
                y = size(relaxData);
                if (y(1) ~= 0)
