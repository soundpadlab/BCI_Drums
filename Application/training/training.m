...
% handles    structure with handles and user data (see GUIDATA)
global smileData clenchData furrowData browData blinkData;
set(handles.recordSwitch, 'Enable', 'off');
set(handles.recordSwitch, 'String', 'recording..');
...
switch get(handles.action, 'Value')
    ...
    case 3
        userData = 'Furrow';
        set (handles.action, 'UserData', userData);
        set (handles.status, 'String', strcat('Furrow eyebrows',' now'));
        furrowData = trainingModule(handles);
    case 4
        userData = 'Brow';
        set (handles.action, 'UserData', userData);
        set (handles.status, 'String', strcat('Raise eyebrows',' now'));
...
set(handles.recordSwitch, 'Enable', 'on');
set(handles.recordSwitch, 'String', 'record');
set (handles.status, 'String', 'recorded!');
